...
// y de Estadísticas. Es una capa sobre cualquier pantalla: abrirla no interrumpe la partida.
// El panel rápido de sonido comparte los mismos ajustes (se mantienen sincronizados).
import { app } from './app.js';
import { $, esc } from './dom.js';
import { t, getLang } from '../i18n/index.js';
import { prefs, setPref, THEMES, TRACKS } from './prefs.js';
import { SFX, MUSIC, sfx, sfxApplyVolumes, musicStart, musicStop, musicRefresh, sndSave } from '../audio/sfx.js';
import { loadRecords, resetRecords, REC_MODES } from './records.js';
...
function statsHTML() {
  ...
  const lv = Object.entries(r.levels).sort((a, b) => a[0] - b[0]).map(([i, b]) => {
    ...
    return `<tr><td class="n">${+i + 1}</td><td>${esc(levelName(L) || t('story.untitled'))}</td><td>${esc(t('stats.turnsShort', { n: b.turns }))}</td><td>${esc(t('stats.strokesShort', { n: b.strokes }))}</td></tr>`;
  }).join('');
